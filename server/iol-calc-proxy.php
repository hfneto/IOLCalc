 ...
 * Description: Proxy seguro para leitura de laudos de biometria por IA (Anthropic). A chave da API fica no servidor. O app entra com e-mail e senha do WordPress (/wp-json/iol/v1/login) e usa um token; a senha de acesso compartilhada continua aceita. Endpoint: /wp-json/iol/v1/read
 * Version: 2.2.0
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('iol/v1', '/login', array(
        'methods'  => 'POST',
        'callback' => 'iolp_login',
        'permission_callback' => '__return_true',
    ));
...
/* ============================================================
 * LOGIN DO APP: e-mail + senha do WordPress -> token de longa duração
 * (guardado como hash no user meta; expira em 180 dias; até 10 por usuário)
 * ============================================================ */
define('IOLP_TOKEN_TTL', 180 * DAY_IN_SECONDS);

function iolp_login(WP_REST_Request $req) {
    $body = $req->get_json_params();
    if (!is_array($body)) {
        return new WP_REST_Response(array('error' => 'Requisição inválida.'), 400);
    }
    $email = isset($body['email']) ? sanitize_text_field((string) $body['email']) : '';
    $pass  = isset($body['password']) ? (string) $body['password'] : '';
    if ($email === '' || $pass === '') {
        return new WP_REST_Response(array('error' => 'Informe e-mail e senha.'), 400);
    }
    // proteção simples contra força bruta: 10 tentativas por IP a cada 15 min
    $ip  = isset($_SERVER['REMOTE_ADDR']) ? (string) $_SERVER['REMOTE_ADDR'] : '0';
    $key = 'iolp_login_' . md5($ip);
    $tries = (int) get_transient($key);
    if ($tries >= 10) {
        return new WP_REST_Response(array('error' => 'Muitas tentativas. Aguarde 15 minutos.'), 429);
    }
    $user = wp_authenticate($email, $pass);
    if (is_wp_error($user)) {
        set_transient($key, $tries + 1, 15 * MINUTE_IN_SECONDS);
        return new WP_REST_Response(array('error' => 'E-mail ou senha incorretos.'), 401);
    }
    delete_transient($key);
    $secret = wp_generate_password(40, false, false);
    $token  = $user->ID . '.' . $secret;
    $list   = get_user_meta($user->ID, 'iolp_tokens', true);
    if (!is_array($list)) $list = array();
    $now = time();
    // remove expirados e limita a 10 dispositivos
    $list = array_values(array_filter($list, function ($t) use ($now) { return isset($t['exp']) && $t['exp'] > $now; }));
    if (count($list) >= 10) array_shift($list);
    $list[] = array('hash' => hash('sha256', $secret), 'exp' => $now + IOLP_TOKEN_TTL, 'ua' => substr((string) $req->get_header('user-agent'), 0, 120), 'created' => $now);
    update_user_meta($user->ID, 'iolp_tokens', $list);
    return new WP_REST_Response(array(
        'token' => $token,
        'name'  => $user->display_name,
        'email' => $user->user_email,
        'expires' => $now + IOLP_TOKEN_TTL,
    ), 200);
}

/** Valida um token emitido por iolp_login. Devolve o WP_User ou null. */
function iolp_user_from_token($token) {
    $parts = explode('.', (string) $token, 2);
    if (count($parts) !== 2 || !ctype_digit($parts[0])) return null;
    $list = get_user_meta((int) $parts[0], 'iolp_tokens', true);
    if (!is_array($list)) return null;
    $h = hash('sha256', $parts[1]);
    foreach ($list as $t) {
        if (isset($t['hash'], $t['exp']) && $t['exp'] > time() && hash_equals($t['hash'], $h)) {
            return get_user_by('id', (int) $parts[0]);
        }
    }
    return null;
}
...
function iolp_read(WP_REST_Request $req) {
    ...
    // autenticação: token do login (app) OU senha de acesso compartilhada (legado)
    $token = isset($body['token']) ? (string) $body['token'] : '';
    if ($token !== '') {
        if (!iolp_user_from_token($token)) {
            return new WP_REST_Response(array('error' => 'Sessão expirada. Entre novamente no app.', 'code' => 'token_invalid'), 401);
        }
    } else {
